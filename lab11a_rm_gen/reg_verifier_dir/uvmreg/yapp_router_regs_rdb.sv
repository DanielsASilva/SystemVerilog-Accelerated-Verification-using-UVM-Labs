...
class ctrl_reg_c extends cdns_uvm_reg;
  ...
  covergroup wr_fld_covg;
    ...
    rsvd_0: coverpoint rsvd_0.value[1:0];
  endgroup
  covergroup rd_fld_covg;
    plen: coverpoint plen.value[5:0];
    rsvd_0: coverpoint rsvd_0.value[1:0];
...
class en_reg_c extends cdns_uvm_reg;
  ...
  covergroup wr_fld_covg;
    ...
    rsvd_0: coverpoint rsvd_0.value[0:0];
...
class yapp_mem_c extends uvm_mem;
  ...
  function new(input string name="yapp_mem");
...
class yapp_pkt_mem_c extends uvm_mem;
  ...
  function new(input string name="yapp_pkt_mem");
...
class yapp_regs_c extends cdns_uvm_reg_block;
  ...
  virtual function void build();
    uvm_reg  reg_set[$];
    default_map = create_map(get_name(), 'h1000, 1, UVM_LITTLE_ENDIAN, 1);
    begin
       uvm_reg_config_ta ta = get_reg_config("yapp_router_regs.yapp_regs");
  ...
  function new(input string name="yapp_regs");
...
class yapp_router_regs_t extends cdns_uvm_reg_block;
  ...
  uvm_reg_map router;
  rand yapp_regs_c yapp_regs;
  rand yapp_mem_c yapp_mem;
  rand yapp_pkt_mem_c yapp_pkt_mem;

  virtual function void build();
    router = create_map("router", 'h0, 1, UVM_LITTLE_ENDIAN, 1);
    default_map = router;
    ...
    yapp_regs = yapp_regs_c::type_id::create("yapp_regs", , get_full_name());
    yapp_regs.configure(this);
    yapp_regs.build();

    //Mapping router map
    yapp_regs.default_map.add_parent_map(router,'h1000);
    router.set_submap_offset(yapp_regs.default_map, 'h1000);
    router.add_mem(yapp_mem, `UVM_REG_ADDR_WIDTH'h1100);
    router.add_mem(yapp_pkt_mem, `UVM_REG_ADDR_WIDTH'h1010);
    set_hdl_path_root("top.dut");    //Apply hdl_paths
